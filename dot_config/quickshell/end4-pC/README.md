...
### 🧩 Edit desktop widget positions

You can move desktop widgets without editing JSON by hand:

1. Right-click an empty area of the desktop.
2. Choose **Edit layout** from the menu.
3. Drag any visible widget to the position you want. A snap grid and center guides help with alignment.
4. Right-click the desktop again and choose **Lock layout** when you are done.

The position is saved automatically in `~/.config/illogical-impulse/config.json`, so it survives a Quickshell restart. This mode also makes widgets that use automatic placement movable; releasing one changes it to a manually positioned widget. If a widget is disabled, enable it from the **Widgets** submenu first.
...
- **[@ilyamiro](https://github.com/ilyamiro)** — for [Serpantinum](https://github.com/ilyamiro/serpantinum), whose wallpaper picker UI inspired the skewed carousel view 🖼️
