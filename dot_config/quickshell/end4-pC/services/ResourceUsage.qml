...
Singleton {
    ...
    Timer {
        // 采样间隔必须跟配置走。这里曾经写成 interval: 1（1 毫秒）且 repeat: true，
        // 等于每秒上千次 reload /proc + 正则匹配 + 重建历史数组，持续抢占事件循环；
        // 平时只是白烧 CPU，一旦有 OBS 录制这类重负载，就会表现为整个 shell 卡顿。
        interval: Math.max(200, Config?.options.resources.updateInterval ?? 3000)
        running: true
        repeat: true
        onTriggered: {
            fileMeminfo.reload()
