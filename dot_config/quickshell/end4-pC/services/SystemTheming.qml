...
Singleton {
    ...
    function templateEnabled(name) {
        const raw = Config.options.appearance.matugenDisabled
        const list = raw ? raw.split(",").map(s => s.trim()).filter(s => s !== "") : []
    ...
    function setTemplateEnabled(name, on) {
        ...
        const next = on
            ...
            : list.concat([name])
        Config.options.appearance.matugenDisabled = next.join(",")
