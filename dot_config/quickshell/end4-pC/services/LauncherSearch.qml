...
Singleton {
    ...
    // 搜索防抖：results 的计算（全量 fuzzy + 逐结果建 QObject）成本不低，
    // 逐键同步算在系统繁忙时（如 OBS 编码抢占 CPU）会放大成肉眼可见的输入延迟。
    // query 保持即时（输入框、前缀图标切换不受影响），results 跟随
    // debouncedQuery；清空时立即同步，收起无延迟。
    ...
    property list<var> results: {
        ...
        ////////////////// Skip? //////////////////
        if (root.debouncedQuery == "")
            return [];

        ///////////// Special cases ///////////////
        if (root.debouncedQuery.startsWith(Config.options.search.prefix.clipboard)) {
            // Clipboard
            const searchString = StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.clipboard);
        ...
        } else if (root.debouncedQuery.startsWith(Config.options.search.prefix.emojis)) {
            // Emojis
            const searchString = StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.emojis);
        ...
        } else if (root.debouncedQuery.startsWith(Config.options.search.prefix.keybinds ?? "<")) {
            // Keybinds
            const searchString = StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.keybinds ?? "<");
        ...
        } else if (root.debouncedQuery.startsWith(Config.options.search.prefix.symbols)) {
            // Material Symbols
            const searchString = StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.symbols);
        ...
        // 应用结果封顶：fuzzy 一个字符能命中几百个应用，逐个建 QObject
        // 是打字卡顿的大头（渲染只展示前几十个），截断到 50 足够覆盖视野。
        const appResultObjects = AppSearch.fuzzyQuery(StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.app)).slice(0, 50).map(entry => {
            return resultComp.createObject(null, {
        ...
        ////////////////// Settings search //////////////////
        const settingsQuery = root.debouncedQuery.toLowerCase().trim();
        ...
        const settingsResults = root.settingsIndex.reduce((acc, page) => {
            ...
            const query = root.debouncedQuery.toLowerCase().trim();
            if (query === "") return acc;
        ...
        const commandResultObject = resultComp.createObject(null, {
            name: StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.shellCommand).replace("file://", ""),
            ...
            iconType: LauncherSearchResult.IconType.Material,
            execute: () => {
                let cleanedCommand = root.debouncedQuery.replace("file://", "");
                ...
                Quickshell.execDetached(["bash", "-c", root.debouncedQuery.startsWith('sudo') ? `${Config.options.apps.terminal} fish -C '${cleanedCommand}'` : cleanedCommand]);
            }
        });
        const webSearchResultObject = resultComp.createObject(null, {
            name: StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.webSearch),
            verb: Translation.tr("Search"),
            type: Translation.tr("Web search"),
            iconName: 'travel_explore',
            iconType: LauncherSearchResult.IconType.Material,
            execute: () => {
                let query = StringUtils.cleanPrefix(root.debouncedQuery, Config.options.search.prefix.webSearch);
        ...
        const launcherActionObjects = root.allActions.map(action => {
            ...
            if (actionString.startsWith(root.debouncedQuery) || root.debouncedQuery.startsWith(actionString)) {
                return resultComp.createObject(null, {
                    name: root.debouncedQuery.startsWith(actionString) ? root.debouncedQuery : actionString,
                    verb: Translation.tr("Run"),
                    type: Translation.tr("Action"),
                    iconName: 'settings_suggest',
                    iconType: LauncherSearchResult.IconType.Material,
                    execute: () => {
                        action.execute(root.debouncedQuery.split(" ").slice(1).join(" "));
                    }
                });
            }
            return null;
        ...
        let result = [];
        const startsWithNumber = /^\d/.test(root.debouncedQuery);
        const startsWithMathPrefix = root.debouncedQuery.startsWith(Config.options.search.prefix.math);
        const startsWithShellCommandPrefix = root.debouncedQuery.startsWith(Config.options.search.prefix.shellCommand);
        const startsWithWebSearchPrefix = root.debouncedQuery.startsWith(Config.options.search.prefix.webSearch);
