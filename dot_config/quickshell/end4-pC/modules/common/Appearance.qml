...
Singleton {
    ...
    // 视频壁纸不能直接当图片用（QML 会报「不支持的图像格式」），
    // 需要统一退回缩略图；这里是全局唯一的判定入口。
    readonly property bool wallpaperIsVideo: {
        const p = String(Config.options.background.wallpaperPath ?? "").toLowerCase();
        return p.endsWith(".mp4") || p.endsWith(".webm") || p.endsWith(".mkv")
            || p.endsWith(".avi") || p.endsWith(".mov");
    }
    readonly property string wallpaperDisplayPath: wallpaperIsVideo
        ? (Config.options.background.thumbnailPath || Config.options.background.wallpaperPath)
