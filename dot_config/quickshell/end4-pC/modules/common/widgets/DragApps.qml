...
Item {
    ...
    Repeater {
        ...
        delegate: Item {
            ...
            // DesktopEntries 为异步加载：shell 启动时创建委托时模型尚未就绪，
            // 若不订阅加载完成信号重查，deskEntry 将永远为 null（点击无响应的根因）
            Connections {
            ...
            DockButton {
                ...
                onClicked: {
                    ...
                    if (!entry || entry.toplevels.length === 0) {
                        // 实时回退查找，防止属性尚未刷新
                        (slotItem.deskEntry ?? DesktopEntries.heuristicLookup(slotItem.appId))?.execute()
                        return
                    }
                ...
                middleClickAction: () => { (slotItem.deskEntry ?? DesktopEntries.heuristicLookup(slotItem.appId))?.execute() }
