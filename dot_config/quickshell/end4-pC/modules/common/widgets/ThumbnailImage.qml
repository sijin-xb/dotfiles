...
StyledImage {
    ...
    Process {
        ...
        command: {
            ...
            if (isVideo) {
                // Extract a frame as video thumbnail via ffmpeg
                return ["bash", "-c",
                    `mkdir -p '${FileUtils.trimFileProtocol(root.thumbnailPath).replace(/\/[^/]+$/, "")}' && [ -f '${FileUtils.trimFileProtocol(root.thumbnailPath)}' ] && exit 0 || { ffmpeg -y -ss 1 -i '${root.sourcePath}' -frames:v 1 -vf "scale=${maxSize}:${maxSize}:force_original_aspect_ratio=decrease" '${FileUtils.trimFileProtocol(root.thumbnailPath)}' && exit 1; }`
                ]
            } else {
                return ["bash", "-c",
