...
LazyLoader {
    ...
    // 点击固定模式：pinned 由外部持有（点击切换），clickMode 打开后不再跟随悬停。
    // 默认值保持旧行为（纯悬停），不影响其它使用 StyledPopup 的组件。
    property bool pinned: false
    property bool clickMode: false
    // 弹窗自身是否被悬停，供固定模式的「移出自动收起」使用
    property bool popupHovered: false

    // 显式转成 bool：原来写 `hoverTarget && hoverTarget.containsMouse` 会在
    // hoverTarget 为空时把 null/undefined 赋给 bool 属性，报 Unable to assign
    readonly property bool hasTarget: hoverTarget !== null && hoverTarget !== undefined
    readonly property bool hoverActive: !clickMode && root.hasTarget && hoverTarget.containsMouse === true
    active: root.hasTarget && (pinned || hoverActive)
