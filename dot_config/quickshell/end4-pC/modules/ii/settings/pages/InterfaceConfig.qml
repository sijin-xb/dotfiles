...
ContentPage {
    ...
    ColumnLayout {
        ...
        ContentSection {
            id: settingsColorGeneration
            icon: "colors"
            title: Translation.tr("Color generation")
            shape: MaterialShape.Shape.VerySunny

            // Target on/off switches. These are read by switchwall.sh and the
            // cursor generator; "Shell & utilities" is the master switch that
            // gates the whole matugen run.
            GroupedList {
                ConfigSwitch {
                    buttonIcon: "hardware"
                ...
                ConfigSwitch {
                    ...
                    checked: Config.options.appearance.wallpaperTheming.enableCursor
                    onCheckedChanged: { Config.options.appearance.wallpaperTheming.enableCursor = checked }
                }
            }

            ContentSubsection {
                visible: Config.options.appearance.wallpaperTheming.enableAppsAndShell
                title: Translation.tr("Terminal options")
            ...
            // Per-app template switches, driven by SystemTheming. Each entry
            // toggles one [templates.*] block in ~/.config/matugen/config.toml.
            // NOTE: 这里不能把 Repeater 放进 GroupedList —— GroupedList 的
            // `list<Item> items` 只会把 Repeater 本身算作一个 0 高度的 item，
            // 展开出的 delegate 高度不计入 implicitHeight，会让页面
            // contentHeight 偏小、Flickable 一往下滚就回弹。
            // 所以按 BarConfig/BackgroundConfig 的模式手写分组列表。
            ContentSubsection {
                visible: Config.options.appearance.wallpaperTheming.enableAppsAndShell
                             && SystemTheming.matugenTemplates.length > 0
                ...
                ColumnLayout {
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            readonly property bool isLast: index === SystemTheming.matugenTemplates.length - 1

                            Layout.fillWidth: true
                            implicitHeight: templateSwitch.implicitHeight + 24
                            color: Appearance.colors.colLayer1
                            ...
                            ConfigSwitch {
                                ...
                                buttonIcon: "widgets"
                                text: Translation.tr("matugen.%1").arg(templateRow.modelData.name)
                                checked: SystemTheming.templateEnabled(templateRow.modelData.name)
                                onCheckedChanged: SystemTheming.setTemplateEnabled(templateRow.modelData.name, checked)
                            }
                        }
                    }
                }
                ...
                StyledText {
                    Layout.fillWidth: true
                    Layout.topMargin: 4
                    text: Translation.tr("Names come from ~/.config/matugen/config.toml.orig, which the shell treats as the source of truth. Turning a template off removes it from the generated config on the next wallpaper change.")
                    font.pixelSize: Appearance.font.pixelSize.smaller
                    color: Appearance.colors.colSubtext
                    opacity: 0.7
