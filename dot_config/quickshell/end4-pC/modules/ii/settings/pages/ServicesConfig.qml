...
ContentPage {
    ...
    ColumnLayout {
        ...
        // ── 录屏 ────────────────────────────────────────────────────────────────
        // 三项都持久化在 config.json 的 screenRecord 下（字段定义见
        // modules/common/Config.qml），scripts/videos/record.sh 用 jq 直接读
        // 同一份配置 —— 所以这里改完不必重启，下一次录制就生效。
        // 灵动岛收起态的捕获条上也有同一组开关（点一下循环切换），共享同一份状态。
        ContentSection {
            icon: "screen_record"
            ...
            GroupedList {
                ConfigSelectionArray {
                    text: Translation.tr("Capture target")
                    icon: "crop_free"
                    currentValue: Config.options.screenRecord.captureTarget
                    options: [
                        { displayName: Translation.tr("Screen"), icon: "monitor",    value: "screen" },
                        { displayName: Translation.tr("Region"), icon: "select_all", value: "region" },
                        { displayName: Translation.tr("Window"), icon: "web_asset",  value: "window" }
                    ]
                    onSelected: newValue => {
                        Config.options.screenRecord.captureTarget = newValue;
                    }
                }
                ConfigSelectionArray {
                    text: Translation.tr("Audio source")
                    icon: "mic"
                    // 存储上是两个布尔量，对外呈现成一个三选一。
                    // 两个同时开着时（捕获条上可以做到）这里按「系统声优先」显示，
                    // 和 record.sh 挑 --audio 的顺序保持一致。
                    currentValue: Config.options.screenRecord.audioSystem
                        ? "system"
                        : (Config.options.screenRecord.audioMic ? "mic" : "none")
                    options: [
                        { displayName: Translation.tr("None"),         icon: "volume_off", value: "none" },
                        { displayName: Translation.tr("System Audio"), icon: "volume_up",  value: "system" },
                        { displayName: Translation.tr("Microphone"),   icon: "mic",        value: "mic" }
                    ]
                    onSelected: newValue => {
                        Config.options.screenRecord.audioSystem = (newValue === "system");
                        Config.options.screenRecord.audioMic    = (newValue === "mic");
                    }
                }
                ConfigSelectionArray {
                    ...
                    options: [
                        ...
                        { displayName: Translation.tr("Medium"), icon: "speed", value: "medium" },
                        { displayName: Translation.tr("High"),   icon: "speed", value: "high" }
                    ]
                    onSelected: newValue => {
