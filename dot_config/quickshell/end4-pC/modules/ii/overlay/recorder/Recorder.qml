...
StyledOverlayWidget {
    ...
    // 获取主显示器缩放比例，用于提升截图分辨率
    readonly property real captureScale: {
        const screen = Quickshell.screens[0];
        return screen ? (WM.monitorGeometry(screen)?.scale ?? 1) : 1;
    }
        ...
        ColumnLayout {
            ...
            Row {
                ...
                BigRecorderButton {
                    ...
                    onClicked: {
                        GlobalStates.overlayOpen = false;
                        // 使用显示器原生缩放比例捕获，提升截图清晰度
                        Quickshell.execDetached(["bash", "-c", `grim -s ${root.captureScale} - | wl-copy`]);
                    }
                }
