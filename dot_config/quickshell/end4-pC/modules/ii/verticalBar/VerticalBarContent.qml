...
Item {
    ...
    function shouldPaintMaterialPill(name) {
        ...
        // island：与水平 Bar 同理（见 BarContent.shouldPaintMaterialPill），
        // 岛屿胶囊由 custom-island/IslandHost.qml 自己绘制，这里不重复画。
        // 注意岛屿面板本身是水平布局，垂直 Bar 下不建议把 island 加进中间区。
        const blacklist = ["workspaces", "divisor", "powerButton", "media", "docktoPanel", "leftSidebarButton", "island"];
