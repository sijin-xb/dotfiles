...
Item {
    ...
    StyledText {
        ...
        // 播放器不给专辑标签时整行收起，不显示 "Unknown album" 占位。
        // 实测 moekoemusic（Electron，MPRIS bus 名带 chromium）只发
        // title / artist / artUrl / length，xesam:album 是空串。
        // 这张卡用的是 anchors（不是 Layout），visible:false 不会让位，
        // 所以额外把高度收成 0，否则下面的 artist 会被顶出一条空隙。
        readonly property bool hasAlbum: ((Players.active?.trackAlbum ?? "") + "").trim().length > 0
        visible: hasAlbum
        height: hasAlbum ? implicitHeight : 0
