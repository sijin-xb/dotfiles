...
Item {
    ...
    Item {
        ...
        // Recording pill (dynamic island) — floats above the bar when recording
        // 录屏指示已统一到顶部灵动岛（主岛或左侧伴随指示器），这里不再重复显示。
        // 如需恢复栏上的浮动录屏胶囊，把 active 改回 Persistent.states.record.enable。
        Loader {
            id: recordingPillLoader
            active: false
            visible: active
            anchors {
            ...
            sourceComponent: Rectangle {
                ...
                RowLayout {
                    ...
                    StyledText {
                        id: timerText
                        text: recordingPill.formatTime(recordingPill.elapsed)
                    ...
                    RippleButton {
                        ...
                        colBackgroundHover: Qt.lighter(Appearance.colors.colError, 1.15)
                        colRipple: "#ffffff"
                        Layout.alignment: Qt.AlignVCenter
                ...
                Timer {
                    interval: 1000
                    repeat: true
                    running: recordingPill.visible
                    onTriggered: recordingPill.elapsed++
                }

                MouseArea {
                    anchors.fill: parent
                    onClicked: {} // absorb clicks so pill doesn't fall through to bar
                }
            }
        }
