...
MouseArea {
    ...
    hoverEnabled: true
    onClicked: {
        // Videos are applied directly: previewing one would just show the
        // stale configured thumbnail until the switch catches up
        if (root.isVideo || GlobalStates.wallpaperSelectorTarget === "lockWall" || !Config.options.background.enableWallpaperPreview)
            root.activated()
        else
