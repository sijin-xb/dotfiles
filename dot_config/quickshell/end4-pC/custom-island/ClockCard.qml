...
// ─────────────────────────────────────────────────────────────────────────
// 从 Brain_Shell 的 src/services/home/ClockCard.qml 移植。
//
// 改动：
//   1. 删除 import "../../" 与 import "../../components"（扁平化后同目录隐式导入）；
//   2. 其余内容逐字保留 —— 时钟大数字排版、Timer 圆环 Canvas、Alarm 列表与
//      新建面板、Stopwatch、底部 TabSwitcher 全部未改（779 行完整移植）。
//
// 依赖（全部同目录）：
//   StatCard / TabSwitcher / TimeInput（TimeInput 在 end4-pC 原本不存在，
//   已从 Brain_Shell 的 src/components/TimeInput.qml 一并补移植到本目录）；
//   ClockState 为并行工作提供的同名 shim 单例，原样引用；
//   提醒走 notify-send，不依赖任何 end4-pC 服务。
// ─────────────────────────────────────────────────────────────────────────
...
StatCard {
    ...
    Item {
            ...
            Column {
                ...
                Item {
                    ...
                    Column {
                        ...
                        Text {
                            anchors.horizontalCenter: parent.horizontalCenter
                            text: Translation.tr("remaining"); font.pixelSize: 8
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        Text {
                            anchors.centerIn: parent
                            text: root._timerRunning ? Translation.tr("Pause") : Translation.tr("Start")
            ...
            Item {
                ...
                Item {
                    ...
                    Text {
                        ...
                        text: Translation.tr("Alarms"); font.pixelSize: 12; font.weight: Font.DemiBold
            ...
            Column {
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        Text {
                            anchors.centerIn: parent
                            text: root._swRunning ? Translation.tr("Stop") : Translation.tr("Start")
                            font.pixelSize: 10; font.weight: Font.Medium
                            color: Theme.active
                        }
        ...
        TabSwitcher {
            ...
            // 这里的 icon 是 Nerd Font 字形，不是 Material 图标名，必须换字体
            iconFont: Theme.nerdFontFamily
            model: [
                { key: "clock",     icon: "󰥔", label: Translation.tr("Clock")     },
                { key: "timer",     icon: "󱎫", label: Translation.tr("Timer")     },
                { key: "alarm",     icon: "󰀠", label: Translation.tr("Alarm")     },
                { key: "stopwatch", icon: "󰔚", label: Translation.tr("Stopwatch") }
            ]
