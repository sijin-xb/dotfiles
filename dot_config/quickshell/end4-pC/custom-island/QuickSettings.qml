...
// Translation：界面文案走 i18n（翻译表见 translations/*.json）
import qs.services

// ─────────────────────────────────────────────────────────────────────────
// 从 Brain_Shell 的 src/services/home/QuickSettings.qml 移植。
//
// 改动：
//   1. 删除 import "../../"、import "../../components"、import "../"
//      （扁平化后同目录隐式导入；Quickshell 保留是给 Quickshell.env 用的）；
//   2. 唯一的服务降级：IpcManager（end4-pC 无此单例）—— 见文件中部
//      「降级说明」注释，原代码保留为注释，不影响卡片自身交互；
//   3. 其余内容逐字保留（1034 行）—— 亮度滑块、开关网格、Tile 样式、
//      Filter 着色器选择弹层、所有轮询 Timer 全部未改。
//
// 依赖：
//   StatCard（同目录）；ShellState / Popups / ScreenRecService / WallpaperService
//   是并行工作提供的同名 shim 单例，原样引用；各开关的实际动作仍是原来的
//   nmcli / bluetoothctl / rfkill / hyprctl / pgrep 命令行，没有换成 end4-pC 服务。
// ─────────────────────────────────────────────────────────────────────────
...
StatCard {
    ...
    // ── 降级说明（IpcManager）───────────────────────────────────────────────
    // 原代码用 Brain_Shell 的 IpcManager 单例（src/state/IpcManager.qml）接收
    // `focusToggleRequested` 信号，让 `qs ipc call ... focus ...` 也能切换焦点模式。
    //
    // end4-pC 不存在跨组件的 IpcManager 单例：它的 IPC 是每个组件自己挂
    // IpcHandler（如 GlobalStates.qml、modules/ii/bar/Bar.qml），也没有名为
    // focus 的等价 target，因此这段无法转接，只能停用（不是隐藏控件 ——
    // 「Focus Mode」开关本身照旧可用，见 _focusToggle / 网格里的 tile）。
    // 若以后在 end4-pC 侧补了等价 IPC，把下面的注释解开即可。
    //
    // Connections {
    //     target: IpcManager
    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: Translation.tr("Brightness"); font.pixelSize: 9; font.weight: Font.Bold
        ...
        Text {
            ...
            text: Translation.tr("Quick Settings"); font.pixelSize: 9; font.weight: Font.Bold
            color: Qt.rgba(Theme.active.r, Theme.active.g, Theme.active.b, 0.55)
        }
        Item { width: parent.width; height: 8 }
            ...
            Flickable {
                ...
                component TglBtn: Rectangle {
                    ...
                    Column {
                        ...
                        Text {
                            ...
                            // btn.icon 是 Nerd Font 字形，不指定字体族会渲染成豆腐块
                            font.family: Theme.nerdFontFamily
                ...
                Grid {
                    ...
                    TglBtn {
                        ...
                        icon: "󰀃"
                        label: Translation.tr("Hotspot")
                        sublabel: root.hotspotLabel
                        onToggled: root._hotspotToggle()
                    }
                    TglBtn {
                        width: tileGrid.btnW; height: tileGrid.btnH
                        on: root.nightLightOn; icon: "󰖐"; label: Translation.tr("Night Light")
                        onToggled: root._nightLightToggle()
                    }
                    TglBtn {
                        width: tileGrid.btnW; height: tileGrid.btnH
                        on: root.caffeineOn; icon: "󰅶"; label: Translation.tr("Caffeine")
                        onToggled: root._caffeineToggle()
                    }
                    TglBtn {
                        width: tileGrid.btnW; height: tileGrid.btnH
                        on: ShellState.focusMode
                        icon: ShellState.focusMode ? "󱃕" : "󰍻"; label: Translation.tr("Focus Mode")
                    ...
                    TglBtn {
                        ...
                        label: ScreenRecService.recording ? Translation.tr("Recording") : Translation.tr("Screen Capture")
                        onToggled: {
