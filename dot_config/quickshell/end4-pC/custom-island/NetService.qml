// ─────────────────────────────────────────────────────────────────────────────
// 从 Brain_Shell 移植：src/services/system/NetService.qml
// 改动：无（只有标准 import），正文逐字保留。
// 用途：给 NetStatsPanel 提供真实接口名与上下行速率。
// 说明：end4-pC 自带 Network 服务（nmcli 系），但它的 API 形状与本文件不同
//       （没有 upSpeed/downSpeed 这类格式化速率），所以这里不转接、直接照搬。
// ─────────────────────────────────────────────────────────────────────────────
