...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            # Extract frame for color generation (skipping first 3 seconds to avoid black frames)
            thumbnail="$THUMBNAIL_DIR/$(basename "$imgpath").jpg"
            if [ ! -f "$thumbnail" ]; then
                ffmpeg -y -ss 00:00:03 -i "$imgpath" -vframes 1 "$thumbnail" 2>/dev/null
                # Fallback to first frame if video is shorter than 3s or extraction failed
                if [ ! -f "$thumbnail" ] || [ ! -s "$thumbnail" ]; then
